...
import { getClothesDryerData, cleanItemName } from '@/app/lib/getClothesDryerData';
import { RankingCard } from '@/app/components/clothes-dryer/RankingCard';
import { FaqSection } from '@/app/components/clothes-dryer/FaqSection';

const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL || 'https://my-only-fragrance-gos.vercel.app';

export async function generateMetadata(): Promise<Metadata> {
  const data = getClothesDryerData();
  const { pageMeta } = data;

  return {
    title: pageMeta.title,
    description: pageMeta.description,
    alternates: { canonical: `${SITE_URL}/clothes-dryer` },
    openGraph: {
      title: pageMeta.title,
      description: pageMeta.description,
      url: `${SITE_URL}/clothes-dryer`,
      type: 'website',
      locale: 'ja_JP',
    },
    twitter: {
...
export default function ClothesDryerPage() {
  const data = getClothesDryerData();
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: '衣類乾燥機 比較ランキング', item: `${SITE_URL}/clothes-dryer` },
    ],
  };
      ...
      <main className="max-w-3xl mx-auto px-4 pb-16">
        ...
        <section className="pt-8 pb-6">
          <nav aria-label="パンくずリスト" className="text-xs text-gray-400 mb-3">
            ...
            <span className="text-gray-600">衣類乾燥機 比較ランキング</span>
          </nav>
            ...
            <div className="flex gap-4 items-start">
              {ranking[0].imageUrl && (
                <a
                  ...
                  className="relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden bg-white border border-purple-200 block"
                >
                  <Image
        ...
        {/* ランキング */}
        <section aria-label="衣類乾燥機 ランキング TOP10" className="mb-12">
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="bg-gray-100 text-gray-600 text-left">
                  ...
                  <th className="px-3 py-3 font-semibold text-center">方式</th>
                  <th className="px-3 py-3 font-semibold text-center">消費電力</th>
                ...
                {ranking.map((item, i) => (
                  <tr key={item.itemUrl} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-3 text-center text-gray-600">
                      {item.specs.power_w ? `${item.specs.power_w}W` : '−'}
                    </td>
        ...
        {/* FAQ */}
        <section aria-label="衣類乾燥機 よくある質問" className="mb-12">
          <h2 className="text-xl font-bold text-gray-900 mb-2">よくある質問（FAQ）</h2>
          <p className="text-xs text-gray-500 mb-5">衣類乾燥機についてのよくある疑問をまとめました</p>
          <FaqSection faqs={faq} />
        </section>
