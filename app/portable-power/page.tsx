...
import { getProcessedData, cleanItemName } from '@/app/lib/getPortablePowerData';
import { RankingCard } from '@/app/components/portable-power/RankingCard';
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${SITE_URL}/portable-power` },
    openGraph: {
      title: pageMeta.title,
      description: pageMeta.description,
      url: `${SITE_URL}/portable-power`,
      type: 'website',
      locale: 'ja_JP',
    },
    twitter: {
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'ホーム', item: SITE_URL },
      {
        '@type': 'ListItem',
        position: 2,
        name: 'ポータブル電源 比較ランキング',
        item: `${SITE_URL}/portable-power`,
      },
    ],
  };
      ...
      <main className="max-w-3xl mx-auto px-4 pb-16">
        ...
        <section className="pt-8 pb-6">
          <nav aria-label="パンくずリスト" className="text-xs text-gray-400 mb-3">
            ...
            <span className="text-gray-600">ポータブル電源 比較ランキング</span>
          </nav>
        ...
        {/* ===== 1位ピックアップ（AIスニペット狙い） ===== */}
        {ranking.length > 0 && (
          <section className="bg-yellow-50 border border-yellow-200 rounded-2xl p-5 mb-8">
            ...
            <div className="flex gap-4 items-start">
              {ranking[0].imageUrl && (
                <a
                  ...
                  className="relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden bg-white border border-yellow-200 block"
                >
                  <Image
        ...
        {/* ===== ランキング ===== */}
        <section aria-label="ポータブル電源 ランキング TOP10" className="mb-12">
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="bg-gray-100 text-gray-600 text-left">
                  ...
                  <th className="px-3 py-3 font-semibold text-center">出力</th>
                ...
                {ranking.map((item, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-center text-blue-700">
                      {item.specs.capacity_wh ? `${item.specs.capacity_wh}Wh` : '−'}
                    </td>
                    <td className="px-3 py-3 text-center text-green-700">
                      {item.specs.output_w ? `${item.specs.output_w}W` : '−'}
                    </td>
        ...
        {/* ===== FAQ（AIエージェント直接回答用） ===== */}
        <section aria-label="ポータブル電源 よくある質問" className="mb-12">
          <h2 className="text-xl font-bold text-gray-900 mb-2">
            よくある質問（FAQ）
          </h2>
          <p className="text-xs text-gray-500 mb-5">
            ポータブル電源についてのよくある疑問をまとめました
          </p>
          <FaqSection faqs={faq} />
        </section>
