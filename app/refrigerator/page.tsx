...
import { getRefrigeratorData, cleanItemName } from '@/app/lib/getRefrigeratorData';
import { RankingCard } from '@/app/components/refrigerator/RankingCard';
import { FaqSection } from '@/app/components/refrigerator/FaqSection';

const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL || 'https://my-only-fragrance-gos.vercel.app';

export async function generateMetadata(): Promise<Metadata> {
  const data = getRefrigeratorData();
  const { pageMeta } = data;

  return {
    title: pageMeta.title,
    description: pageMeta.description,
    alternates: { canonical: `${SITE_URL}/refrigerator` },
    openGraph: {
      title: pageMeta.title,
      description: pageMeta.description,
      url: `${SITE_URL}/refrigerator`,
      type: 'website',
      locale: 'ja_JP',
    },
    twitter: {
      ...
      <main className="max-w-3xl mx-auto px-4 pb-16">
        ...
        <section className="pt-8 pb-6">
          <nav aria-label="パンくずリスト" className="text-xs text-gray-400 mb-3">
            ...
            <span className="text-gray-600">大型冷蔵庫 比較ランキング</span>
          </nav>
        ...
        {/* 1位ピックアップ */}
        {ranking.length > 0 && (
          <section className="bg-cyan-50 border border-cyan-200 rounded-2xl p-5 mb-8">
            ...
            <div className="flex gap-4 items-start">
              {ranking[0].imageUrl && (
                <a
                  ...
                  className="relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden bg-white border border-cyan-200 block"
                >
                  <Image
        ...
        {/* ランキング */}
        <section aria-label="大型冷蔵庫 ランキング TOP10" className="mb-12">
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="bg-gray-100 text-gray-600 text-left">
                  ...
                  <th className="px-3 py-3 font-semibold text-center">容量</th>
                ...
                {ranking.map((item, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-center text-blue-700">
                      {item.specs.capacity_l ? `${item.specs.capacity_l}L` : '−'}
                    </td>
                    <td className="px-3 py-3 text-center text-green-700">
                      {item.specs.energy_saving_rate ? `${item.specs.energy_saving_rate}%` : '−'}
                    </td>
        ...
        {/* FAQ */}
        <section aria-label="大型冷蔵庫 よくある質問" className="mb-12">
          <h2 className="text-xl font-bold text-gray-900 mb-2">
            よくある質問（FAQ）
          </h2>
          <p className="text-xs text-gray-500 mb-5">
            大型冷蔵庫についてのよくある疑問をまとめました
          </p>
          <FaqSection faqs={faq} />
        </section>
