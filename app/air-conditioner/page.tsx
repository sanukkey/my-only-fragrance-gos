...
import { getAirConditionerData, cleanItemName } from '@/app/lib/getAirConditionerData';
import { RankingCard } from '@/app/components/air-conditioner/RankingCard';
import { FaqSection } from '@/app/components/air-conditioner/FaqSection';

const SITE_URL =
  process.env.NEXT_PUBLIC_SITE_URL || 'https://my-only-fragrance-gos.vercel.app';

export async function generateMetadata(): Promise<Metadata> {
  const data = getAirConditionerData();
  const { pageMeta } = data;

  return {
    title: pageMeta.title,
    description: pageMeta.description,
    alternates: { canonical: `${SITE_URL}/air-conditioner` },
    openGraph: {
      title: pageMeta.title,
      description: pageMeta.description,
      url: `${SITE_URL}/air-conditioner`,
      type: 'website',
      locale: 'ja_JP',
    },
    twitter: {
...
export default function AirConditionerPage() {
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'エアコン 比較ランキング', item: `${SITE_URL}/air-conditioner` },
    ],
  };
      ...
      <main className="max-w-3xl mx-auto px-4 pb-16">
        ...
        <section className="pt-8 pb-6">
          <nav aria-label="パンくずリスト" className="text-xs text-gray-400 mb-3">
            ...
            <span className="text-gray-600">エアコン 比較ランキング</span>
          </nav>
        ...
        {/* 1位ピックアップ */}
        {ranking.length > 0 && (
          <section className="bg-sky-50 border border-sky-200 rounded-2xl p-5 mb-8">
            ...
            <div className="flex gap-4 items-start">
              {ranking[0].imageUrl && (
                <a
                  ...
                  className="relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden bg-white border border-sky-200 block"
                >
                  <Image
        ...
        {/* ランキング */}
        <section aria-label="エアコン ランキング TOP10" className="mb-12">
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="bg-gray-100 text-gray-600 text-left">
                  ...
                  <th className="px-3 py-3 font-semibold text-center">対応畳数</th>
                  <th className="px-3 py-3 font-semibold text-center">省エネ</th>
                  <th className="px-3 py-3 font-semibold text-center">APF</th>
                ...
                {ranking.map((item, i) => (
                  <tr key={item.itemUrl} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-3 text-center text-blue-700">
                      {item.specs.tatami ? `${item.specs.tatami}畳` : '−'}
                    </td>
                    <td className="px-3 py-3 text-center text-green-700">
                      {item.specs.energy_saving_pct ? `${item.specs.energy_saving_pct}%` : '−'}
                    </td>
        ...
        {/* FAQ */}
        <section aria-label="エアコン よくある質問" className="mb-12">
          <h2 className="text-xl font-bold text-gray-900 mb-2">よくある質問（FAQ）</h2>
          <p className="text-xs text-gray-500 mb-5">エアコンについてのよくある疑問をまとめました</p>
          <FaqSection faqs={faq} />
        </section>
