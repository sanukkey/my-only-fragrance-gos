...
import { RefrigeratorRankingItem } from '@/app/types/refrigerator';
import { cleanItemName } from '@/app/lib/getRefrigeratorData';

interface Props {
  item: RefrigeratorRankingItem;
}
    ...
    <article className="flex gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          {item.specs.capacity_l && (
            <span className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full">
              {item.specs.capacity_l}L
            </span>
          )}
          {item.specs.energy_saving_rate && (
            <span className="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full">
              省エネ{item.specs.energy_saving_rate}%
            </span>
          )}
          {item.specs.annual_kwh && (
            <span className="text-xs bg-gray-50 text-gray-600 px-2 py-0.5 rounded-full">
              年{item.specs.annual_kwh}kWh
            </span>
          )}
