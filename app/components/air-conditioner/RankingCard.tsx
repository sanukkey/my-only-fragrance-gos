...
import { AirConditionerRankingItem } from '@/app/types/air-conditioner';
import { cleanItemName } from '@/app/lib/getAirConditionerData';

interface Props {
  item: AirConditionerRankingItem;
}
    ...
    <article className="flex gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          ...
          {item.specs.energy_saving_pct && (
            <span className="text-xs bg-green-50 text-green-700 px-2 py-0.5 rounded-full">
              省エネ{item.specs.energy_saving_pct}%
            </span>
          )}
          {item.specs.apf && (
